...
function RetentionOfferModal({
  ...
  const [step, setStep] = useState<'offers' | 'checkout'>('offers')
  ...
  const handleAcceptOffer = async (offer: 'free_month' | 'annual_discount' | 'lifetime_discount') => {
    // For free month, use the existing API
    if (offer === 'free_month') {
      setLoading(offer)
      setError(null)
      try {
    ...
    // For discount offers, show embedded checkout
    const plan = offer === 'annual_discount' ? 'annual' : 'lifetime'
    setSelectedPlan(plan)
    setLoading(offer)
    setError(null)

    try {
      const sessionId = `retention-${plan}-${Date.now()}`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-dark-card border border-dark-border rounded-2xl p-8 max-w-lg w-full mx-4 shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        {step === 'offers' ? (
          <>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-dark-text-muted">
                        Enter code STAY10 at checkout for $53.10/year
                      </p>
                    </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-dark-text-muted">
                        Enter code STAY25 at checkout for $74.25
                      </p>
                    </div>
            ...
            >
              {loading === 'refund' ? (
                ...
                'No thanks, proceed with cancellation request'
              )}
            </button>
          </>
        ) : !stripePromise ? (
          ...
          <div className="overflow-y-auto flex-1">
            ...
            <p className="text-sm text-dark-text-muted mb-4">
              {selectedPlan === 'annual'
                ? 'Enter code STAY10 at checkout for 10% off'
                : 'Enter code STAY25 at checkout for 25% off'}
            </p>
