...
export default function ExtensionDownloadModal({ isOpen, onClose }: ExtensionDownloadModalProps) {
  ...
  const [release, setRelease] = useState<ReleaseInfo>({ version: null, downloadUrl: null, releaseNotes: null })
  ...
  useEffect(() => {
    if (isOpen) {
      setLoading(true)
      fetch('/api/software/latest?platform=extension')
        .then(res => res.json())
        .then(data => {
          setRelease({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-dark-card border border-dark-border rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-6">
          {loading ? (
            <div className="w-full flex items-center justify-center gap-3 bg-primary/50 text-white font-semibold py-4 px-6 rounded-xl">
            ...
            >
              ...
              Download MineGlance Extension {release.version && `v${release.version}`}
              <span className="text-sm opacity-75">(.zip)</span>
            </a>
          ) : (
            <div className="w-full flex items-center justify-center gap-3 bg-red-500/20 text-red-400 font-semibold py-4 px-6 rounded-xl">
              Download not available - please try again later
            </div>
          )}
        </div>
